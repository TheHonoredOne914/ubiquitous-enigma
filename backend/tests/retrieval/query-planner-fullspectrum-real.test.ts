...
test("fullspectrum democratic-space planner has 14 buckets, 80+ unique queries, and low duplication", () => {
  const contract = buildAgendaContract({ requestId: "planner-full", originalUserQuery: prompt, outputDepth: "phd_level" });
...
test("phd democratic-space planner has 60+ unique queries", () => {
  const contract = buildAgendaContract({ requestId: "planner-phd", originalUserQuery: prompt, outputDepth: "phd_level" });
  const plan = buildBucketedQueryPlan(contract, "phd_level");
  const uniqueQueries = new Set(plan.queries.map((query) => query.query.toLowerCase()));
