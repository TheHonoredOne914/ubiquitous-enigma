...
test("mode depth strategy makes fast and phd/full role prompts materially different", () => {
  assert.notEqual(getModeDepthStrategy("fast_research").instruction, getModeDepthStrategy("phd_level").instruction);
  assert.notEqual(
    buildRoleSpecificInstructions("thesis_synthesizer", "fast_research", {}),
    buildRoleSpecificInstructions("thesis_synthesizer", "fullspectrum", {}),
  );
  assert.match(buildRoleSpecificInstructions("thesis_synthesizer", "phd_level", {}), /multi-sentence|contradictions|cross-source/i);
