...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    const safeKey = sanitizeChartKey(key)
...
function sanitizeChartColor(value: string | undefined): string | null {
  ...
  if (/url\s*\(|javascript:|<|>|\{|\}|@import|expression\s*\(/i.test(color)) return null
  if (/^#[0-9a-f]{3,8}$/i.test(color)) return color
  if (/^rgba?\(\s*(?:\d{1,3}\s*,\s*){2}\d{1,3}(?:\s*,\s*(?:0|1|0?\.\d+))?\s*\)$/i.test(color)) return color
  if (/^hsla?\(\s*\d{1,3}(?:deg)?(?:\s+|,\s*)\d{1,3}%(?:\s+|,\s*)\d{1,3}%(?:\s*(?:\/|,)\s*(?:0|1|0?\.\d+|[0-9]{1,3}%))?\s*\)$/i.test(color)) return color
  if (/^var\(--[a-z0-9-_]+\)$/i.test(color)) return color
  return null
}

function sanitizeChartKey(value: string): string | null {
