  ...
  return (
    <div className="mx-4 mb-3 overflow-hidden rounded-xl border border-[#3b6fd4]/25 bg-background/90 shadow-sm">
      <div className="flex items-center gap-3 px-4 py-2.5 border-b border-border/40">
        ...
        {searchTier && (
          <span className="rounded-full border border-[#2a2d38] bg-[#0d0e12] px-2 py-0.5 font-mono text-[10px] text-[#6b6b82]">
            {searchTier}
          </span>
        )}
      </div>
    ...
    <div className="w-full rounded-2xl border border-border/50 bg-background/95 shadow-sm overflow-hidden mb-6 animate-pipeline-card-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 p-4">
        {activeModels.map((key, idx) => {
          ...
          let status: "searching" | "reading" | "done" | "warning" | "error" | "idle" = "idle";
          let percent = 0;
          ...
          } else if (terminalStatus.terminal && !terminalStatus.success) {
            status = terminalStatus.severity === "error" ? "error" : "warning";
            percent = 100;
            statusLabel = terminalStatus.label;
          } else if (phase === "complete" || isComplete) {
            status = "done"; percent = 100; statusLabel = "Complete";
          } else if (phase === "synthesizing" || phase === "verifying") {
            status = "done"; percent = 100; statusLabel = "Reading done";
          } else if (fetchingTotal > 0 && fetchedCount < fetchingTotal) {
            status = "reading";
            percent = Math.round((fetchedCount / fetchingTotal) * 100);
      ...
      {activeModels.some(k => {
        ...
        <div className="mx-4 mb-3 rounded-xl border border-border/40 overflow-hidden divide-y divide-border/30">
          {activeModels.map((key, idx) => {
            const data    = getModelData(key);
            const meta    = modelMetaFromKey(key);
            const persona = getPersona(idx);
      ...
      {(isDiscussing || discussion || isSynthesizing) && (
        <div className="mx-4 mb-3 rounded-xl border border-slate-300/40 bg-background/90 shadow-sm overflow-hidden">
      ...
      {/* ── Streaming answer ────────────────────────────────────────────────── */}
      {visibleAnswer && (
        <div className="mx-4 mb-4 rounded-xl border border-slate-300/40 bg-background/95 shadow-sm p-4">
